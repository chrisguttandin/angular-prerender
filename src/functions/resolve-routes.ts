...
export const resolveRoutes = (routesWithParameters: { parameterValueMaps: IParameterValuesMap[]; route: string }[]) =>
    routesWithParameters.flatMap(({ route, parameterValueMaps }) => {
        ...
        for (const parameterValueMap of parameterValueMaps) {
            const numberOfPermutations = Object.values(parameterValueMap).reduce((sum, values) => sum * values.length, 1);

            for (let i = 0; i < numberOfPermutations; i += 1) {
                resolvedRoutes.push(
                    Object.entries(parameterValueMap)
